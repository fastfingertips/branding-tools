...
class ReleaseBuilder:
    ...
    def _should_exclude(self, path: Path) -> bool:
        ...
        name = path.name
        matches_pattern = any(
            name.endswith(p[1:]) if p.startswith("*") else name == p for p in self.EXCLUDE_PATTERNS
        )
    ...
    def build_all(
        self, include_firefox: bool = True, gecko_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """Run all build steps."""
        ...
        result = {
            ...
            "paths": {"chrome": self.build_chrome_package(), "github": self.build_github_package()},
